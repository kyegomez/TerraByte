...
from TerraByte.utils.stable_adamw import StableAdamWUnfused
# from optimus_prime import DilatedTransformer, AutoregressiveWrapper, TerraByteEmbedding, Decoder
from data_streaming import DatasetElement

from TerraByte.model.megabyte import DilatedMegabyte, DilatedTransformer

class TrainTerraByte:
    class CFG:
        BATCH_SIZE: int = 16
        ...
        CHECKPOINT_NAME: str = 'TerraByte.pt'
    ...
    def activation_checkpointing(
        ...
        check_fn = lambda submodule: isinstance(submodule, DilatedTransformer)
    ...
    def build_dataloaders():
        ...
        tokenized_dataset = dataset.map(
            ...
            batched=True,
            num_proc=TrainTerraByte.CFG.NUM_CPU,
            remove_columns=["text"]
        )

        block_size = TrainTerraByte.CFG.SEQ_LEN
        ...
        train_dataset = tokenized_dataset.map(
            group_texts, batched=True, num_proc=TrainTerraByte.CFG.NUM_CPU,
        )
    ...
    def Train():
        ...
        accelerator = Accelerator(
            gradient_accumulation_steps=TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY,
        ...
        accelerator.init_trackers(
            ...
            config={
                "batch_size": TrainTerraByte.CFG.BATCH_SIZE,
                "gradient_accumulate_every": TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY,
                "learning_rate": TrainTerraByte.CFG.LEARNING_RATE,
                "seq_len": TrainTerraByte.CFG.SEQ_LEN
            },
            init_kwargs={"wandb": {"entity": TrainTerraByte.CFG.ENTITY_NAME}},
        )

        accelerator.print(f"Total GPUS: {accelerator.num_processes}")
        set_seed(TrainTerraByte.CFG.SEED)

        model = DilatedMegabyte()

        model.to(accelerator.device)

        TrainTerraByte.print_num_params(model, accelerator)

        if TrainTerraByte.CFG.USE_FSDP:
            model = TrainTerraByte.fsdp(
                model,
                mp="fp16",
                shard_strat="SHARD_GRAD"
            )

        if TrainTerraByte.CFG.USE_ACTIVATION_CHECKPOINTING:
            TrainTerraByte.activation_checkpointing(model, accelerator)

        model = accelerator.prepare(model)

        # if TrainTerraByte.CFG.USE_PRETOKENIZED:
        #     train_dataset = TrainTerraByte.build_pre_tokenized()
        # else:
        #     train_dataset = TrainTerraByte.build_dataloaders()

        # train_loader = DataLoader(
        #     train_dataset, batch_size=TrainTerraByte.CFG.BATCH_SIZE, collate_fn=default_data_collator,
        # )

        optim = TrainTerraByte.decoupled_optimizer(
            model=model,
            learning_rate=TrainTerraByte.CFG.LEARNING_RATE, 
            weight_decay=TrainTerraByte.CFG.WEIGHT_DECAY, 
            beta_1=0.90, 
            beta_2=0.95, 
            optimizer_type='lion',
            use_fsdp=True,
        ...
        max_train_steps = math.ceil(TrainTerraByte.CFG.MAX_STEPS / TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY)
        
        # max_train_steps = math.ceil(len(train_loader) / TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY)
        accelerator.print(f"Max train steps: {max_train_steps}")

        NUM_WARMUP_STEPS = int(max_train_steps * 0.01)
        accelerator.print(f"Num warmup steps: {NUM_WARMUP_STEPS}")

        if False: # TrainTerraByte.CFG.USE_DEEPSPEED:
            ...
            lr_scheduler = TrainTerraByte.get_lr_scheduler_with_warmup(
                optimizer=optim,
                scheduler_type='cosine',
                num_warmup_steps=NUM_WARMUP_STEPS,
                max_train_steps=max_train_steps,
                grad_accumulate_every=TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY,
            )
        ...
        batches_num = np.floor(TrainTerraByte.CFG.MAX_STEPS / TrainTerraByte.CFG.BATCH_SIZE)
        
        max_train_steps = math.ceil(batches_num / TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY)
        accelerator.print(f'Max train steps recalculated: {max_train_steps}')

        total_batch_size = (
            TrainTerraByte.CFG.BATCH_SIZE * accelerator.num_processes * TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY
        )
        ...
        if TrainTerraByte.CFG.RESUME_FROM_CHECKPOINT:
            accelerator.print(f'Resuming from checkpoint {TrainTerraByte.CFG.RESUME_FROM_CHECKPOINT}')
            accelerator.load_state(os.path.join(TrainTerraByte.CFG.OUTPUT_PATH))

            training_difference = os.path.splitext(path)[0]

            resume_step = (
                int(training_difference.replace('step_', ''))
                * TrainTerraByte.CFG.GRADIENT_ACCUMULATE_EVERY
            )

        if TrainTerraByte.CFG.RESUME_FROM_CHECKPOINT and resume_step is not None:
        ...
        dataset = DatasetElement(tokenizer, sequence_length=TrainTerraByte.CFG.SEQ_LEN, batch_size=TrainTerraByte.CFG.BATCH_SIZE)
        ...
        while not training_done:
            ...
            if isinstance(TrainTerraByte.CFG.CHECKPOINTING_STEPS, int):
                if completed_steps % TrainTerraByte.CFG.CHECKPOINTING_STEPS == 0:
                    output_dir = f'step_{completed_steps}'
                    if TrainTerraByte.CFG.OUTPUT_DIR is not None:
                        output_dir = os.path.join(TrainTerraByte.CFG.OUTPUT_DIR, output_dir)
        ...
        if TrainTerraByte.CFG.OUTPUT_DIR is not None:
            ...
            with accelerator.main_process_first():
                accelerator.save(
                    unwrapped_model.state_dict(), f'{TrainTerraByte.CFG.OUTPUT_DIR}/final/final_model.pt'
                )
